...
El [http://es.wikipedia.org/wiki/GNU_Compiler_Collection](GNU Compiler Collection) (colección de compiladores GNU) es un conjunto de [compiladores](http://es.wikipedia.org/wiki/Compilador) creados por el proyecto [GNU](http://es.wikipedia.org/wiki/Compilador). GCC es [software libre](http://es.wikipedia.org/wiki/Software_libre) y lo distribuye la [Free Software Foundation (FSF)](http://es.wikipedia.org/wiki/FSF) bajo la licencia general pública [GPL](http://es.wikipedia.org/wiki/Licencia_p%C3%BAblica_general_de_GNU).
